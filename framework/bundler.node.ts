...
export async function bundle({
	...
	for (const [outputPath, output] of Object.entries(
		...
		const relativePath = path.relative(browserOutdir, outputPath);
	...
	for (const [outputPath, output] of Object.entries(
		...
		if (!output.entryPoint) continue;
		const relativePath = path.relative(ssrOutdir, outputPath);
		const inputFile = path.resolve(root, relativePath);
		const inputFileWithoutExt = inputFile.slice(0, inputFile.lastIndexOf("."));
	...
	for (const [outputPath, output] of Object.entries(
		...
		if (!output.entryPoint) continue;
		const relativePath = path.relative(serverOutdir, outputPath);
		const inputFile = path.resolve(root, relativePath);
		const inputFileWithoutExt = inputFile.slice(0, inputFile.lastIndexOf("."));
